...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x, gamma, beta, normalize, sample_mean, sample_var, eps)
        ...
        # No need to update running mean and variance during test-time

        # Store the intermediate values in cache for the backward pass
        cache = (x, gamma, beta, normalize, running_mean, running_var, eps)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, gamma, beta, normalize, running_mean, running_var, eps = cache

    dgamma = np.sum(dout * normalize, axis = 0)
    dbeta = np.sum( dout, axis = 0)
    
    dnorm = dout * gamma
    dvar = np.sum(dnorm * (x - running_mean) * (-0.5) * ((running_var + eps) ** (-1.5)), axis = 0)
    dmean = np.sum (dnorm * (-1.0 * np.sqrt(running_var + eps)), axis = 0) + dvar * (np.sum(-2.0 * (x - running_mean), axis = 0) / x.shape[0])

    dx = dnorm * (1.0 / np.sqrt(running_var + eps)) + dvar * (2.0 * (x- running_mean) / x.shape[0]) + ( dmean / x.shape[0]) 
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def batchnorm_backward_alt(dout, cache):
    ...
    # Calculate dgamma and dbeta as before
    dgamma = np.sum(dout * normalize, axis=0)
    dbeta = np.sum(dout, axis=0)
    
    # Calculate dx using a simplified expression
    dx = (1.0 / N) * gamma * (running_var + eps) ** (-0.5) * (
          N * dout - np.sum(dout, axis=0) - normalize * np.sum(dout * normalize, axis=0))
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    cache = (scaled_x, normalize_x, gamma, 1./np.sqrt(feature_var + eps), np.sqrt(feature_var + eps))
    return out, cache
...
def layernorm_backward(dout, cache):
    ...
    N = dout.shape[0]
    scaled_x, normalized_x, gamma, ivar, sqrtvar = cache
    ...
    dx = dx.T  # Transpose it back to the shape of (N, D)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    stride = conv_param.get('stride', 1)  # 1 is the default value if 'stride' is not present
    pad = conv_param.get('pad', 0)        # 0 is the default value if 'pad' is not present
        ...
        for f in range(F):
            for dhh in range(dh):
                for dww in range(dw):
                    ...
                    x_slice = padded_x[n, :, row_start: row_end, col_start: col_end]
                    out[n, f, dhh, dww] = np.sum(x_slice * w[f]) + b[f]
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def max_pool_forward_naive(x, pool_param):
    ...
    N, C, H, W = x.shape
    stride = pool_param.get('stride', 1)  # 1 is the default value if 'stride' is not present
    pool_height = pool_param.get('pool_height', 1)        # 0 is the default value if 'pool_height' is not present
    pool_width = pool_param.get('pool_width', 1)        # 0 is the default value if 'pool_height' is not present
    htag = int(1 + (H - pool_height) / stride)
    wtag = int(1 + (W - pool_width) / stride)
    out = np.zeros((N, C, htag, wtag))
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    outt, cache = batchnorm_forward(x_reshaped.reshape(-1, x_reshaped.shape[-1]), gamma, beta, bn_param)
    out = outt.reshape(x_reshaped.shape).transpose([0, 3, 1, 2])
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dxx, dgamma, dbeta = batchnorm_backward(dout_t.reshape(-1, dout_t.shape[-1]),cache)
    dx = dxx.reshape(dout_t.shape).transpose([0, 3, 1, 2])
